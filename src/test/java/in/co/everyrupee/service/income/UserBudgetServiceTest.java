...
public class UserBudgetServiceTest {
	...
	try {
	    ...
	    Mockito.when(userBudgetRepository.fetchUserBudgetWithCategoryIds(categoryIdList, FINANCIAL_PORTFOLIO_ID,
		    getDateMeantFor())).thenReturn(getUserBudgetList());
	...
	UserBudget userBudget = getUserBudgetService().saveAutoGeneratedUserBudget(formData, FINANCIAL_PORTFOLIO_ID,
		false);
	verify(getUserBudgetRepository(), times(0)).save(Mockito.any(UserBudget.class));
	...
	UserBudget userBudget = getUserBudgetService().saveAutoGeneratedUserBudget(formData, FINANCIAL_PORTFOLIO_ID,
		true);
	verify(getUserBudgetRepository(), times(1)).save(Mockito.any(UserBudget.class));
	assertThat(userBudget).isNotNull();
	assertThat(userBudget.getPlanned()).isEqualTo(600);
    }

    /**
     * TEST: Save Auto Generated User Budget With Amount (AutoGenerates = FALSE)
     */
    @Test
    public void saveUserBudget() {
	MultiValueMap<String, String> formData = new LinkedMultiValueMap<String, String>();
	formData.add(DashboardConstants.Budget.PLANNED, "600");
	formData.add(DashboardConstants.Budget.DATE_MEANT_FOR, DATE_MEANT_FOR);
	formData.add(DashboardConstants.Budget.FINANCIAL_PORTFOLIO_ID, FINANCIAL_PORTFOLIO_ID);
	formData.add(DashboardConstants.Budget.CATEGORY_ID, "3");
	formData.add(DashboardConstants.Budget.AUTO_GENERATED_JSON, "false");

	UserBudget userBudgetReturned = new UserBudget();
	userBudgetReturned.setAutoGeneratedBudget(false);
	userBudgetReturned.setPlanned(600);
	when(getUserBudgetRepository().save(Mockito.any(UserBudget.class))).thenReturn(userBudgetReturned);
	// Auto generation = false
	UserBudget userBudget = getUserBudgetService().saveAutoGeneratedUserBudget(formData, FINANCIAL_PORTFOLIO_ID,
		true);
	...
	assertThat(userBudget).isNotNull();
	assertThat(userBudget.getPlanned()).isEqualTo(600);
	assertThat(userBudget.getAutoGeneratedBudget()).isEqualTo(false);
    }
     ...
     */
    @Test
    public void updateAutoGeneratedUserBudgets() {
	Map<Integer, Double> categoryIdAndCategoryTotalMap = new HashMap<Integer, Double>();
	categoryIdAndCategoryTotalMap.put(3, 300d);
	getUserBudgetService().updateAutoGeneratedUserBudget(FINANCIAL_PORTFOLIO_ID, categoryIdAndCategoryTotalMap,
		DATE_MEANT_FOR);
