...
isWindows ? ($('.sidebar .sidebar-wrapper, .main-panel').perfectScrollbar(), $('html').addClass('perfect-scrollbar-on'))  : $('html').addClass('perfect-scrollbar-off');
var breakCards = !0,
searchVisible = 0,
transparent = !0,
transparentDemo = !0,
...
$(document).ready(function () {
  ...
  1 == breakCards && $('[data-header-animation="true"]').each(function () {
    ...
    a.mouseenter(function () {
      ...
      20 <= hover_count && $(this).children('.card-header, .card-header-image').addClass('hinge animated')
    })
  }),
...
$(document).on('click', '.navbar-toggler', function () {
  if ($toggle = $(this), 1 == mobile_menu_visible) $('html').removeClass('nav-open'),
  $('.close-layer').remove(),
  setTimeout(function () {
...
md = {
  ...
  checkFullPageBackgroundImage: function () {
    ...
    void 0 !== image_src && (image_container = '<div class="full-page-background" style="background-image: url(' + image_src + ') "/>', $page.append(image_container))
  },
  ...
  initRightMenu: debounce(function () {
    ...
    mobile_menu_initialized ? 991 < $(window).width() && ($sidebar_wrapper.find('.navbar-form').remove(), $sidebar_wrapper.find('.nav-mobile-menu').remove(), mobile_menu_initialized = !1)  : ($navbar = $('nav').find('.navbar-collapse').children('.navbar-nav'), mobile_menu_content = '', nav_content = $navbar.html(), nav_content = '<ul class="nav navbar-nav nav-mobile-menu">' + nav_content + '</ul>', navbar_form = $('nav').find('.navbar-form').get(0).outerHTML, $sidebar_nav = $sidebar_wrapper.find(' > .nav'), $nav_content = $(nav_content), $navbar_form = $(navbar_form), $nav_content.insertBefore($sidebar_nav), $navbar_form.insertBefore($nav_content), $('.sidebar-wrapper .dropdown .dropdown-menu > li > a').click(function (e) {
      e.stopPropagation()
